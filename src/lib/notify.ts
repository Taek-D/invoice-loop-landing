...
export async function sendLeadNotification(input: LeadNotificationInput) {
  ...
  await resend.emails.send({
    from,
    to,
    subject: `[청구루프] 새 베타 신청 - ${input.name}`,
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #1f2937;">
        ...
        <p><strong>현재 워크플로우</strong>: ${input.currentWorkflow}</p>
        <p><strong>가장 큰 불편</strong>: ${input.biggestPain}</p>
        ...
        <p><strong>UTM Campaign</strong>: ${input.utmCampaign ?? "-"}</p>
        <p><strong>Referrer</strong>: ${input.referrer ?? "-"}</p>
      </div>
    `,
  });
}
